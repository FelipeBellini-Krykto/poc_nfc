...
echo "Iniciando build do app Android - $flavor"

cd ../base_app/ && \
rm -rf android/.gradle && \
flutter clean && \
flutter build $build_type --obfuscate --split-debug-info=build/app/outputs/symbols --flavor "$flavor" -t lib/main_"$flavor".dart

echo "Termino da geração build do app Android - $flavor"